...
function renderField(field, highlightMonth) {
  if (!field.length) return '<div class="empty">（場札なし）</div>';
  return field.map(id => {
...
function renderCaptured(ids) {
  ...
  return order.map(t => `
    <div class="cap-row cap-${t}">
      <span class="cap-label">${({hikari:'光',tane:'種',tanzaku:'短',kasu:'カス'})[t]}</span>
      <div class="cap-cards">
...
function renderYakuList(yaku) {
  if (!yaku.length) return '<span class="dim">役なし</span>';
...
export function render(view, mySide, handlers) {
  ...
  $('#scoreboard').innerHTML = `
    <div class="sb-row">
      ...
      <span>親: ${view.players[view.oya]}</span>
      <span>山札: ${s.yamaCount}</span>
    </div>
    <div class="sb-row">
  ...
  $('#opp-info').innerHTML = `
    <div class="player-name">${oppName} ${s.turn === opp ? '<span class="badge">手番</span>' : ''}
      ${s.koiCalled[opp] ? '<span class="badge koi">こいこい</span>' : ''}</div>
    <div class="yaku-row">${renderYakuList(s.yaku[opp])}
      <span class="pts">合計 ${totalPoints(s.yaku[opp])} 文</span></div>
  `;
  ...
  $('#me-info').innerHTML = `
    <div class="player-name">${myName} ${isMyTurn ? '<span class="badge mine">あなたの手番</span>' : ''}
      ${s.koiCalled[mySide] ? '<span class="badge koi">こいこい</span>' : ''}</div>
    <div class="yaku-row">${renderYakuList(s.yaku[mySide])}
      <span class="pts">合計 ${totalPoints(s.yaku[mySide])} 文</span></div>
  `;
  ...
  if (s.phase === 'play_hand' && isMyTurn) {
    bar.innerHTML = `<div class="hint">手札を1枚タップ → 場札を選んで取る</div>`;
  } else if (s.phase === 'pick_hand_pair' && isMyTurn) {
    bar.innerHTML = `<div class="hint">取りたい場札をタップ</div>`;
  } else if (s.phase === 'draw_deck' && isMyTurn) {
    bar.innerHTML = `<button class="btn-primary" id="btn-draw">山札をめくる (${s.yamaCount})</button>`;
    $('#btn-draw').onclick = () => handlers.onDraw();
  } else if (s.phase === 'pick_draw_pair' && isMyTurn) {
    bar.innerHTML = `<div class="hint">引いたカードと合わせる場札をタップ</div>`;
  } else if (s.phase === 'koi_decision' && isMyTurn) {
    bar.innerHTML = `
      <button class="btn-secondary" id="btn-agari">あがり (${totalPoints(s.yaku[mySide])} 文)</button>
      <button class="btn-primary" id="btn-koi">こいこい！</button>
    `;
    ...
    bar.innerHTML = `
      <div class="result">
        ${winner ? `${view.players[winner]} の勝ち！ +${s.lastEvent.points} 文` : '引き分け'}
      </div>
      <button class="btn-primary" id="btn-next">次の局へ</button>
    `;
    ...
    bar.innerHTML = `
      <div class="result big">
        ${winner ? `🎉 ${view.players[winner]} 試合勝利！` : '引き分け試合'}
        <br><small>${view.players.host} ${h} - ${g} ${view.players.guest}</small>
      </div>
    `;
  } else {
    bar.innerHTML = `<div class="hint">${view.players[s.turn]} の手番…</div>`;
  }
